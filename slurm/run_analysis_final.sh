...
echo "    OVERWRITTEN by the 1008-grid retrain (confirmed: embed_dim now contains"
echo "    48/96/192, which do not exist in the 256 grid), so regenerating it now"
echo "    would give a 1008-supernet figure that does NOT match this 256 main table."
echo "    → Copy the archived 256 version into $OUT :"
echo "      $ARCHIVE_HINT"
echo "    NOTE: that archived PNG still labels the readmission panel '90d', from"
echo "    before plot_regression_combined.py was changed to 'Readmission (3M)'."
echo "    The fix CANNOT be applied to it without the 256 raw data, so resolve the"
echo "    wording in the figure caption instead ('within 3 months (90 days)')."
